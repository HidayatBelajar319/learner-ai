...
graph TD
    ...
    B --> E[Knowledge Base]
    B --> F[Learning System]
    B --> G[Evaluation System]
    B --> H[Personalization Engine]
    C --> I[Session Context]
    C --> J[User Profile]
    D --> L[Short-term Memory]
    D --> M[Long-term Memory]
    E --> O[Curriculum DB]
    E --> P[Content Generator]
    F --> R[Adaptive Learning]
    G --> T[Quiz Generator]
    H --> X[Recommendation Engine]
    B --> AA[Response Generator]
    AA --> AB[User]
```
...
```mermaid
flowchart TD
    A[Pemilihan Topik] --> B[Penilaian Awal]
    B --> C[Penentuan Level]
    C --> D[Rekomendasi Materi]
    D --> E[Penyajian Materi]
    E --> F[Latihan]
    F --> G[Quiz]
    G --> H[Evaluasi]
    H --> I{Umpan Balik}
    I -->|Paham| J[Materi Berikutnya]
    I -->|Belum Paham| E
    J --> K[Ujian Akhir]
    K --> L[Sertifikat]
```
